...
description: >
  ...
  device management cli, access control cli, partner org, brew install rhombus.
...
For MSP/partner accounts, use `--partner-org` with any command:
...
## Relationship to Rhombus API Skill

This skill covers the CLI tool. For direct API integration (HTTP calls, SDK generation, OpenAPI spec, webhooks), see the `/rhombus-api` skill. The CLI wraps the same API — every auto-generated CLI command maps 1:1 to an API endpoint.
